...
class TutorialRunner {
  ...
  async notifyPhaseChange(phaseKey) {
    if (!this.active || this.cleared) {
      console.log('[notifyPhaseChange] skip phase=' + phaseKey + ' active=' + this.active + ' cleared=' + this.cleared);
      return;
    }
    ...
    const sg = !!(this.scenario && this.scenario.showPhaseGuide);
    const pgt = (this.scenario && this.scenario.phaseGuideTexts) || {};
    const hasText = !!pgt[phaseKey];
    console.log('[notifyPhaseChange] phase=' + phaseKey + ' showGuide=' + sg + ' hasText=' + hasText + ' textKeys=' + Object.keys(pgt).join(','));
    if (sg && hasText && typeof window !== 'undefined' && window._tutorialShowPhaseGuide) {

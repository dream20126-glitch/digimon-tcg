...
export interface EffectBlock {
  ...
  // トリガー条件を2件以上指定した場合の結合方法。未指定/'and'なら従来通り全部満たす必要がある。
  ...
  // JSONへは指定時のみ step.source_type:'digimon'|'option' として出力する。
  // ⚠ 現状どちらもエンジン未対応（hasActiveImmuneEffectsの呼び出し元がeffectSourceTypeを
  // 渡していないため、保存はできるが絞り込みは機能せず常に相手の効果全てをブロックする）
  immuneCardType?: 'digimon' | 'option' | '';
...
export interface MiniStep {
  ...
  groupsOp?: 'and' | 'or';
  // 各グループのアクション/置き先が「共通（ルール本体のアクションを使う）」か
  // 「グループごとに異なる」かの表示切り替え用フラグ（UIの見た目の切り替えが主目的）。
  // true = 共通（各グループのaction/deckPosition/optionsは無視しルール本体を使う。
  //        グループ側の個別アクションUIは非表示）
  // false = グループごとに異なる（ルール本体のアクションUIを非表示にし、各グループの
  //        アクションUIのみ表示。ただし各グループのaction未指定時はルール本体に
  //        フォールバックする既存挙動はそのまま）
...
export interface DictEntry {
  ...
  // トリガー辞書専用: このトリガーの発動主体（自分/相手/両方）に、デジモン/テイマー/カード等の
  // 種別区別が存在しない（例:「デッキが増えたとき」「セキュリティが減ったとき」のようなゾーン系
  // イベント）。スプシ「種別なし」列に "1"/"true" 等が入っていれば true。
  // trueのとき、レシピエディタの発動主体エディタで「デジモン/テイマー」種別ボタンを表示しない
  noSubjectType?: boolean;
}
